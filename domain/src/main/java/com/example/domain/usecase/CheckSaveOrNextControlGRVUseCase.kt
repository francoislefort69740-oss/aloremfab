...
class CheckSaveOrNextControlGRVUseCase(private val controlGRVStepRepository: ControlGRVStepLocalRepository, private val controlGRVLocalRepository: ControlGRVLocalRepository) {
    suspend operator fun invoke(reference: Int?, currentStep: GRVControlStepEnum): ResultOf<Triple<Boolean, Boolean, Boolean>> {
        return try {
            if (reference == null) {
                ...
                val currentStepExist = controlGRVStepRepository.checkIfControlGRVStepExist(reference, type = type)
                val nextStepExist = controlGRVStepRepository.checkIfControlGRVStepExist(reference, type = nextType)
